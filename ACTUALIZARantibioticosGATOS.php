...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql_actualizar = "UPDATE antibioticosgatos SET descripcionproducto=?, fecha_elaboracion=?, fecha_caducidad=?, cantidad=? WHERE nombreDes=?";
    ...
    if (mysqli_stmt_execute($stmt)) {
        ...
        header("Location: M_antivioticosGATOS.php");
        exit;
    } else {
    ...
    $sql_select = "SELECT * FROM antibioticosgatos WHERE nombreDes=?";
    ...
    if ($resultado && mysqli_num_rows($resultado) > 0) {
        ...
        echo "<form method='post' action='ACTUALIZARantibioticosGATOS.php'>";
